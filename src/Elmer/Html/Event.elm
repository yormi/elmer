...
select value =
    TestState.map
        (\context ->
            let
                eventPropagations =
                    [ eventPropagation (eventHandlerQuery "input") (inputEvent value) ]
            in
...
hasHandlersFor context eventPropagations element =
    let
        handlers =
            List.map (\ep -> ep.handlerQuery (Context.render context) element) eventPropagations
                |> List.concat
    in
